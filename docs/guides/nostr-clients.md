...
Buzz is a Nostr relay that speaks NIP-29 (relay-based groups) natively.
Third-party Nostr clients connect directly to `buzz-relay` using NIP-29 and
NIP-42 authentication. This page is the operational how-to; the underlying
protocol semantics are in [Protocol](../architecture/protocol.md).
...
- [Protocol](../architecture/protocol.md) — event shape, kinds, wire messages
- [Buzz NIPs Index](../reference/nips.md) — Buzz protocol extensions
- [Self-Hosting](self-hosting.md) — relay membership (NIP-43) and operator admin
